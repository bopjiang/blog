...
| Amazon Aurora: instances in 1 AZ | >= 99.9% | 存储仍跨3个AZ复制 | 依赖实例恢复 |
| Amazon Aurora: instances in >=2 AZs | >= 99.99% | Region内已提交写入为0 | 有Aurora Replica时通常小于30秒 |
...
* Aurora SLA里的Single-AZ Cluster和Multi-AZ Cluster是AWS的SLA定义: 前者指cluster里的DB instances限制在同一个AZ, 后者指DB instances部署在两个或更多AZ. 这不是说Aurora Single-AZ的数据只在一个AZ; Aurora cluster volume本身仍会把存储副本放在三个AZ.
