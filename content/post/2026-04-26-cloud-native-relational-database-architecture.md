...
更准确地说, 今天TiDB Cloud文档里的资源形态主要是Starter, Essential和Dedicated. 过去常说的TiDB Serverless, 在产品体验上更接近Starter/Essential这条路线: 用户不直接管理底层组件, 也不需要先规划一组固定集群节点, 而是把数据库当成一个按需使用的云服务入口.

更关键的是, TiDB Serverless的底层存储哲学已经不是经典TiDB那种"TiKV本地盘 + Multi-Raft复制"的shared-nothing路线. 公开资料里更接近TiDB X / shared-storage方向: S3这类对象存储成为持久数据的single source of truth, 本地盘或EBS承担cache, WAL, metadata等更靠近写入路径和低延迟访问的角色. 也就是说, 它把很多经典TiDB里由Raft副本和节点迁移承担的弹性问题, 转移到了对象存储和共享缓存层.

从这个角度看, TiDB Serverless在存储架构上确实更像Snowflake, BigQuery或Neon这类cloud-native shared-storage系统: 持久数据放在对象存储, 计算层尽量无状态化, 本地盘主要用于缓存、临时数据和写入路径加速. 但这个类比只能解释存储哲学, 不能解释全部数据库语义. Snowflake主要面向OLAP, 优化重点是大扫描、聚合、列式执行和虚拟仓库隔离; TiDB Serverless仍然要面对MySQL兼容、OLTP事务、索引维护、MVCC和HTAP查询. 换句话说, 它在底层越来越像对象存储优先的云数据系统, 但上层仍然背着关系型事务数据库的包袱.

这改变了TiDB的使用门槛, 也改变了它的存储复杂度边界. 但它没有让分布式数据库的基本物理事实消失: 系统仍然要处理MVCC, 事务冲突, range split, hotspot, 多租户隔离和分布式执行. Serverless化主要是把容量规划、集群运维、连接入口、弹性和计费包装成更轻的产品体验, 而不是让事务协调和冷热数据访问的代价消失.
...
* [How PingCAP transformed TiDB into a serverless DBaaS using Amazon S3 and Amazon EBS](https://aws.amazon.com/blogs/storage/how-pingcap-transformed-tidb-into-a-serverless-dbaas-using-amazon-s3-and-amazon-ebs/)
