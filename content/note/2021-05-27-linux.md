...
```bash
# compress
tar czvf archive.tar.gz xx-dir
...
```bash
# setup a SOCKS5 proxy server listening at TCP/1082, forward request to remote host.
ssh -D 1080  -q -C -N remote-machine-host

# use the SOCKS5 proxy, git can use it.
# https_proxy=socks5://127.0.0.1:1080
```
to use the socks5 proxy in Python, some packages should be installed,  `pip install pysocks `

convert SOCKS5 proxy to HTTP proxy server using privoxy
```bash
$ sudo pip install privoxy

# cat /etc/privoxy/config
# listen-address  192.168.28.6:8118
...
### ssh tunnel
```bash
# Listen on the remote side (127.0.0.1:8069), and forward connection to the local side (0.0.0.0:18069) using ssh tunnel
ssh -fN -L 0.0.0.0:18069:127.0.0.1:8069 txclaw
```
...
```bash
## synchronize entire directory
rsync -au '~/Library/Mobile Documents/iCloud~md~obsidian/Documents/notes/' ./notes
```
...
#  refresh the groups that a logged-in user is in without having to re-login
newgrp docker
```
...
```
dstat -t -a
# dstat is deprecated, use `dool` instead.
dool -t -a
```
